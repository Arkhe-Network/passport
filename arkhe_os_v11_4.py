...
def make_shamir_polynomial(secret: int, threshold: int, n_parties: int, p: int = BLS12_381_P) -> Polynomial:
    """Create a random (threshold, n) Shamir polynomial with given secret."""
    coeffs = [secret % p]
    for _ in range(threshold):
        coeffs.append(secrets.randbelow(p - 1) + 1)
    return Polynomial(coeffs, p)

def lagrange_interpolate(points: List[Tuple[int, int]], p: int) -> int:
...
class FullADKG:
    ...
    async def consensus(self, leader_id: int, round_tag: bytes) -> List[int]:
    ...
    async def execute(self, party_ids: Optional[List[int]] = None) -> ADKGOutput:
        ...
        Q = await self.consensus(leader, round_tag)
...
async def demo_v11_4():
    ...
    try:
        ...
        print("numpy not installed, skipping Discourse Detection demo")
