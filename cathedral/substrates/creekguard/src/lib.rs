...
pub struct CanaryRegistry {
    pub active: BTreeMap<[u8; 16], CanaryToken>,
    detected_count: u64,
}
...
impl TimingCorrelationEngine {
    ...
    pub fn detect_burst_pattern(&self, _now_ms: u64) -> Vec<(AgentId, AgentId, f32)> {
...
pub struct CreekGuard {
    pub entropy_analyzer: (), // Stateless
    pub canary_registry: CanaryRegistry,
    pub fingerprint_registry: FingerprintRegistry,
    pub semantic_limiter: SemanticRateLimiter,
    pub timing_engine: TimingCorrelationEngine,
...
impl CreekGuard {
    ...
    pub fn analyze_message(
        ...
        let (entropy_verdict, _entropy, _chi_sq) = analyze_entropy(&msg.payload);
...
mod tests {
    ...
    fn test_near_duplicate_detection() {
        let mut registry = FingerprintRegistry::new(1);
        let topic: TopicId = "test-topic";
    ...
    fn test_creekguard_pipeline() {
        ...
        let (_verdict2, errors2) = guard.analyze_message(&msg2);
