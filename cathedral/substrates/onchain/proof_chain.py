"""
RecursiveProofChain — Substrato 1100 v1.0.0
Cadeia de provas ZK integradas com verificação de assinaturas.
Selo: PROOF-CHAIN-1100-v1.0.0-2026-06-08
"""
...
class RecursiveProofChain:
    ...
    def get_chain_hash(self) -> str:
        ...
        chain_content = json.dumps({
            ...
            "nodes": [self.nodes[h].proof_hash for h in self.ordered_indices[-100:]],
        }, sort_keys=True)
        return "0x" + hashlib.sha256(chain_content.encode()).hexdigest()

    def verify_chain_integrity(self) -> Tuple[bool, List[str]]:
        errors = []
        for idx_hash in self.ordered_indices:
            node = self.nodes[idx_hash]
            if node.index > 0:
