...
Selo: GOVERNANCE-BRIDGE-1100-v1.0.0-2026-06-08
"""
...
from cathedral.substrates.onchain.types import CanonizationType, MemoryLakeEntry, SignatureStatus
...
from cathedral.substrates.onchain.signer import EIP712Signer, EIP712_TYPES
...
class GovernanceBridge:
    """Bridge que permite ao kernel propor canonizações e aguardar assinatura humana."""

    def __init__(self, signer: EIP712Signer, memory_lake: MemoryLake,
                 proof_chain: RecursiveProofChain, default_deadline_seconds: int = 86400 * 7):
        self.signer = signer
    ...
    def propose_canonization(self, canonization_type: CanonizationType,
                              data: Dict, deadline: Optional[int] = None) -> str:
        proposal_id = "0x" + hashlib.sha256(json.dumps({
            "type": canonization_type.name, "data": data, "timestamp": time.time()
        }, sort_keys=True).encode()).hexdigest()

        proposal = GovernanceProposal(
            proposal_id=proposal_id, proposal_type=canonization_type,
            proposal_data=data, proposed_at=time.time(),
        ...
        logging.info(f"[GovernanceBridge] Proposal {proposal_id[:16]}... | "
    ...
    def submit_signature(self, proposal_id: str, signature: str) -> bool:
        ...
        if proposal.state != ProposalState.PENDING_SIGNATURE:
            logging.error(f"[GovernanceBridge] Not pending: {proposal.state.value}")
            return False
    ...
    def sign_and_canonize_locally(self, proposal_id: str) -> bool:
        """Assina localmente (automatizada). Use com cautela — bypasses human-in-the-loop."""
        proposal = self.proposals.get(proposal_id)
        if not proposal:
            return False
    ...
    def _canonize_proposal(self, proposal: GovernanceProposal) -> bool:
        entry = MemoryLakeEntry(
            entry_hash="", entry_type=proposal.proposal_type, data=proposal.proposal_data,
            ...
            timestamp=time.time(), status=SignatureStatus.VERIFIED_CANONICAL,
        )
    ...
    def get_proposal_status(self, proposal_id: str) -> Optional[Dict]:
        ...
        if not p:
            return None
        return {
            "proposal_id": p.proposal_id, "type": p.proposal_type.name,
            "state": p.state.value, "proposed_at": p.proposed_at,
            "deadline": p.deadline, "expired": p.is_expired(),
            "has_signature": p.signature is not None,
            "canonized": p.state == ProposalState.CANONIZED,
            "lake_entry_hash": p.canonization_entry.entry_hash if p.canonization_entry else None,
            "proof_node_index": p.proof_node.index if p.proof_node else None,
        }
    ...
    def await_signature(self, proposal_id: str, timeout: float = None) -> bool:
        ...
        while True:
            ...
            if not p:
                return False
            if p.state in (ProposalState.SIGNED, ProposalState.CANONIZED):
                return True
            if p.is_expired():
                return False
    ...
    def get_telemetry(self) -> Dict:
        ...
        return {
            "module": "GovernanceBridge", "version": "1.0.0", "substrate": "1100",
