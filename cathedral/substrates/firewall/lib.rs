...
use alloc::vec::Vec;
 // APENAS para debug/logging em builds com alloc; não usado em hot path
use alloc::boxed::Box;
        ...
        match self.state {
            ...
            CircuitState::Open => {
                ...
                if now_ms.saturating_sub(self.last_failure_time_ms) >= self.window_ms {
                    ...
                    // Aqui retornamos false; a transição para HalfOpen é feita externamente
                    false
                } else {
                    false
                }
...
impl ProActBFT {
    ...
    pub fn engage_honeypot(&mut self, agent: AgentId, _now_ms: u64) {
...
pub struct TemporalSemanticFirewall {
    ...
    proact: ProActBFT,
    tee: Option<Box<dyn TeeSecureInference>>, // None em simulação
}
...
mod tests {
    ...
    fn test_circuit_breaker_transitions() {
        ...
        assert!(cb.can_execute(1500)); // Em HalfOpen, permite teste
    }
