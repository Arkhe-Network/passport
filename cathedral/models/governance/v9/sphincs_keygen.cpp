...
void wots_public_key(const uint8_t* seed, uint32_t leaf_idx, uint32_t tree_idx, uint8_t* out) {
    ...
    for (uint32_t i = 0; i < L; ++i) {
        ...
        uint16_t i_u16 = static_cast<uint16_t>(i);
        // Note: Python implementation uses big-endian for the derivations.
        // Assuming C++ is run on little-endian machine, we reverse bytes manually if needed to match,
        // but for simplicity, we just copy raw bytes.
        // If exact compatibility is needed across arch, __builtin_bswap32 etc should be used.
        // We'll mimic Python's big endian representation:
        uint32_t leaf_idx_be = __builtin_bswap32(leaf_idx);
        uint32_t tree_idx_be = __builtin_bswap32(tree_idx);
        uint16_t i_be = __builtin_bswap16(i_u16);

        memcpy(data + N, &leaf_idx_be, 4);
        memcpy(data + N + 4, &tree_idx_be, 4);
        memcpy(data + N + 8, &i_be, 2);
